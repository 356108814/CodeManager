...
class EmailSender(object):
    def __init__(self):
        self._from_name = config_parser.get('email', 'from_name')
        self._from = config_parser.get('email', 'from')
        self._server = config_parser.get('email', 'server')
        self._user = config_parser.get('email', 'user')
        self._password = config_parser.get('email', 'password')
...
if __name__ == '__main__':
    ...
    param = {'title': u'账单异常', 'content': u'下面为异常订单信息：<br>',
             'bills': [{'orderid': 'kkk1'}, {'orderid': 'lll2'}]}
    with open('bill_alarm_email.html', 'r') as f:
        tpl = f.read()
        tpl = tpl.decode('utf-8', 'ignore')
